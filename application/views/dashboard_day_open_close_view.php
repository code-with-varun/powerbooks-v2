...
<section class="content">
		...
		if($pos_status=="OPENED"){$change_status="CLOSED";$pos_status="OPENED"; $btn_color="pink";}
		else{$change_status="OPENED";$pos_status="CLOSED"; $btn_color="green";}
		echo '<center><h4> Billing '.$pos_status.' for date ( '.date('d-M-Y - l',strtotime($current_pos_date)).' )<br><br>';
		if($auto_day_end!='YES'){	
		echo'<form action="update-pos" method="POST"><input type="hidden" name="change_status" value="'.$change_status.'"><button type="submit" class="btn bg-'.$btn_color.' waves-effect">Click Here</button> to '.$change_status.' POS. </h4></center></form>';
		}
		?> 
